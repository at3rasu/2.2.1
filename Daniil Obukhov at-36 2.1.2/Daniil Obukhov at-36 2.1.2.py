...
class Graphic:
    def __init__(self, profession: str, years: List[int], average_salary: List[int],
        ...
        self.__grouped_bar_graph(ax1, "Уровень зарплат по годам", self.average_salary, self.years, self.average_salary_profession, 'средняя зп', f'средняя зп {self.profession}')
    ...
    def __grouped_bar_graph(self, ax, title : str, values_x : List[int], values_y : List[int], values_x2 : List[int], label_x : str, label_x2 : str):
        ...
        width = 0.4
        rects1 = ax.bar(x - width / 2, values_x, width, label=label_x)
        rects2 = ax.bar(x + width / 2, values_x2, width, label=label_x2)
    ...
    def __pie_graph(self, ax):
        ...
        if sum_vacancies != 100:
            vacancies.insert(1, 100 - sum_vacancies)
            cities.insert(1, "Другие")
...
graph = Graphic(profession=input_data[1],
